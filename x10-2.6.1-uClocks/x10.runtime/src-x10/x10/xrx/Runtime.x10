...
//This array records those workers blocked in an advance in the previous phase of a clock, or a when, or anywhere else, and waiting to be woken up to start their work of the current phase.
    //It is a common variable for all threads. Anyone can wake someone from here
    //Using a deque because it already has an efficient implementation. Do not want to add another lock for this list
    public static pendingWorkers:Deque = new Deque();
   
   
    public static def startNewThread():void {
        pool.startNewThread();
    }
    
    public static def getIdleCountAndReduceByNButKeepItNonNegative(n:Int):Int{
        return pool.workers.getIdleCountAndReduceByNButKeepItNonNegative(n);
    }
