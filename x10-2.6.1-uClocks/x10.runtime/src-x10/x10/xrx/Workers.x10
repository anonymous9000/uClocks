...
final class Workers {
    ...
    //Adds the current worker as a spare thread
    def addWorkerAsSpareAndUnparkPendingWorker(worker:Worker,pendingWorker:Worker):void{
    	lock.lock();
    	val i = spareCount++ + idleCount;
    	parkedWorkers(i) = worker;
    	pendingWorker.unpark();
    ...
    //It reduces the value of idleCount by 'N' provided idleCount>N, else it sets idleCount to 0
    //It also returns the initial idleCount
    def getIdleCountAndReduceByNButKeepItNonNegative(n:Int):Int{
    	var idleThreadCountTemporaryVariable:Int; //This variable is necessary because we want to get the idle count and set its value atomically
    	lock.lock();
    	idleThreadCountTemporaryVariable = idleCount;
    	if (idleCount <= n){
    		spareCount += idleCount;
    		idleCount = 0n;
    	} else{
    		idleCount -= n;
    		spareCount += n;
       	...
       	lock.unlock();
    	return idleThreadCountTemporaryVariable;
    }
    ...
    def take(worker:Worker):Activity {
        ...
        if (idleCount >= Runtime.NTHREADS - 1){
            lock.unlock();
            return null;
        }
