...
class Pool {
    ...
    // notify the pool a worker is about to execute a blocking operation
    def startNewThread():void {
        val i = workers.startNewThread();
        if (i > 0n) {
            // if no spare thread is available allocate and start a new thread
            val worker = new Worker(i);
            workers(i) = worker;
    ...
    def scan(random:Random, worker:Worker):Activity {
        ...
        for (;;) {
            ...
            //Check if there are any pending workers blocked in a clock. Try to unpark 1 pending worker and park yourself if successful in getting the 1 worker to unpark
            var pendingWorkerToUnpark:Worker = Runtime.pendingWorkers.steal() as Worker;
            if (pendingWorkerToUnpark!=null){ //If there is a thread in this array (steal() was successful), unpark it and park yourself
            	workers.addWorkerAsSpareAndUnparkPendingWorker(worker,pendingWorkerToUnpark);
            	if (Runtime.worker().activity!=null){
            	    return Runtime.worker().activity;
            	}
            	continue;
            }
